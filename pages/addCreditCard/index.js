...
Page({
    ...
    bindDateChange: function(e) {
        ...
        var cuDa = e.detail.value
        // 截取时间 开始与结束索引 2019-08
        var cuYears = cuDa.substring(2,4);
        var cuMonth = cuDa.substring(5,7)
        console.log("时间", cuDa)
        console.log("截取年", cuYears)
        console.log("截取月", cuMonth)
        this.setData({
            dataShow: false,
            date: cuMonth + cuYears
        })
      },
    //   账单日
    onzhangdanchange: function(e){
        this.setData({
            zhangdan: e.detail
        })
    },
    // 还款日
    onhuankuanchange: function(e){
        this.setData({
            huankuan: e.detail
        })
    },
    ...
    toSave: function(e){
        ...
        if(!this.data.anquanma){
            config.showToast(this.data.textanquan,this.data.iconfail);
            return false;
        }
        if(!this.data.date){
            config.showToast(this.data.textdata,this.data.iconfail);
            return false;
        }
        if(!this.data.zhangdan){
            config.showToast(this.data.textzhangdan,this.data.iconfail);
            return false;
        }
        if(!this.data.huankuan){
            config.showToast(this.data.texthuankuan,this.data.iconfail);
            return false;
        }
        ...
        var resData = {
            ...
            ExpiredDate: this.data.date,
            Cvv2: this.data.anquanma,
            StatementDate: this.data.zhangdan,
            RepaymentDate: this.data.huankuan,
